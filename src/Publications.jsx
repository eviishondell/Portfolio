  ...
  return (
    <div className="font-sans bg-white">
      ...
      <nav className="fixed top-0 left-0 right-0 bg-stone-100 z-50 border-b border-stone-200">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex gap-8 text-sm">
            ...
            <Link to="/projects" className="hover:text-stone-600 transition">
              Projects
            </Link>
            <Link
              to="/publications"
            ...
            <Link to="/about" className="hover:text-stone-600 transition">
              About Me
            </Link>
          </div>
        </div>
      ...
      <div className="snap-y snap-mandatory overflow-y-scroll h-screen">
        ...
        <section
          ref={heroRef}
          className="bg-white snap-start snap-always min-h-screen flex items-center justify-center"
        >
          <div className="max-w-4xl mx-auto text-center px-6 space-y-6">
            <h1
              className={`text-5xl md:text-7xl lg:text-8xl font-semibold text-stone-900 tracking-tight leading-none transition-all duration-1000 ${
            ...
            <p
              className={`text-xl md:text-2xl text-stone-600 font-normal max-w-3xl mx-auto transition-all duration-1000 delay-500 ${
                heroVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
              }`}
            >
              Peer-reviewed research in robotics, human-computer interaction, and
              accessible technology design.
            </p>

            <div
              className={`flex flex-col sm:flex-row items-center justify-center gap-4 pt-6 transition-all duration-1000 delay-1000 ${
                heroVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
              }`}
            >
              <a
        ...
        <section
          ref={conferenceRef}
          id="conference"
          className="bg-stone-100 snap-start snap-always min-h-screen flex items-center py-20"
        >
          <div className="max-w-5xl mx-auto px-6 w-full">
            <div
              className={`text-center mb-16 transition-all duration-1000 ${
                conferenceVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
              }`}
            >
            ...
            <div className="space-y-6">
              {conferencePublications.map((pub, idx) => (
                <div
                  key={pub.id}
                  className={`group bg-white rounded-2xl p-6 md:p-8 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 border border-stone-200 ${
                    conferenceVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
                  }`}
                ...
                >
                  <div className="flex justify-between items-start gap-6">
                    <div className="flex-grow">
                      <span className="text-xs font-semibold text-stone-600 mb-3 inline-block">
                        {pub.year}
                      </span>
                      <h3 className="font-semibold text-xl md:text-2xl text-stone-900 mb-3 leading-snug">
                        {pub.title}
                      </h3>
        ...
        <section
          ref={workshopRef}
          id="workshop"
          className="bg-white snap-start snap-always min-h-screen flex items-center py-20"
        >
          <div className="max-w-5xl mx-auto px-6 w-full">
            <div
              className={`text-center mb-16 transition-all duration-1000 ${
                workshopVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
              }`}
            >
              <h2 className="text-4xl md:text-6xl font-semibold text-stone-900 mb-4">
                Workshop Papers.
            ...
            <div className="space-y-6">
              {workshopPublications.map((pub, idx) => (
                <div
                  key={pub.id}
                  className={`group bg-stone-100 rounded-2xl p-6 md:p-8 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 ${
                    workshopVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
                  }`}
                  style={{ transitionDelay: `${(idx + 1) * 150}ms` }}
                >
